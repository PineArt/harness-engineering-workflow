...
In `Lite` and `Full`, the main thread should spend its budget on high-value orchestration: architecture choices, slice boundaries, owner assignment, Opus or Advisor debate, review synthesis, integration decisions, evidence acceptance, and recovery checkpoints. Treat direct implementation by the main thread as boundary pressure, not as the default way to make progress.
`Orchestrator` may make only tiny coordination edits in `Lite` and `Full`, and only when all of these are true: the edit is not assigned to any active `Implementer`, it touches no owned implementation slice, it is one logical coordination change such as a run artifact correction or non-business glue update, and it is recorded in `Decision Log` or the relevant artifact. Anything larger than a small surgical edit, any business logic change, or any change inside an assigned writable area must be delegated to the owning `Implementer`.
Delegation-First Lock: in `Lite` and `Full`, `Orchestrator` may not perform any task-domain action for a slice until a field-valid `Delegation Record` for that slice is written to the run workspace naming a non-`Orchestrator` owner that maps to the current `Role Owner Table`. Task-domain actions include read, search, grep, file inspection, diagnostic work, root-cause analysis, test execution, log review, worktree operation, implementation, verification, publish, commit, check-in, and submit when performed for a task slice. `Orchestrator` may still inspect run-workspace artifacts, role tables, checkpoints, validator output, and owner-produced evidence for coordination, step closure, and integration.
`S3` cannot close until every required task-domain slice has a field-valid `Delegation Record`; diagnostic, implementation, verification, worktree, publish, commit, check-in, and submit slices each need one record. `Expected Evidence` must be an artifact path with locator or a checkpoint name, not vague prose.
...
After auto compact, thread copy, or resume, do not continue from a chat summary alone. Read `CURRENT.md`, open the latest checkpoint, inspect `Boundary Violations`, re-run `validate_harness_run.py <run-workspace>`, then continue from the remaining checklist and evidence pointers. Resolve open `Boundary Violations` before any task-domain action continues.
...
- `Lite` and `Full`: `Orchestrator` creates `CURRENT.md` and a checkpoint file before `S1` validation, refreshes checkpoints at `S1`, `S3`, `S5`, and `S7`, and refreshes before/after delegated work crossing a new independent `Context Boundary` when feasible.
- `Lite` and `Full`: `S3` closure records a checkpoint reflection that no task-domain action preceded the relevant `Delegation Record`; if the reflection finds a possible violation, record it in `Boundary Violations` and return to the owning step before execution proceeds.
...
- For `Lite` and `Full`, `Orchestrator` writes `Delegation Record` artifacts for each diagnostic, implementation, verification, worktree, publish, commit, check-in, or submit slice before any task-domain action for that slice starts.
- Each implementation slice must name its owned files or writable area, plus an explicit `Do Not Touch` boundary when nearby files are likely to tempt incidental edits. While the slice is active, `Orchestrator` must not edit those owned files; it records review comments, follow-up tasks, or integration decisions instead.
...
- If a confirmed `Orchestrator` task-domain action occurred during `S4` through `S6`, `S7 Quality Gate` blocks with a `Boundary Integrity` failure. Resume paths must resolve open `Boundary Violations` before continuing.
...
- one `Context Pack`
- one `Task Graph`
- one `Delegation Record` per task-domain diagnostic, implementation, verification, worktree, publish, commit, check-in, or submit slice
